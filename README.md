...
| 🥋 **승급 이력** | 그랄·벨트 받은 날을 남기면 지금 벨트가 맞춰집니다 |
| 📈 **승급 진행도** | *(선택)* 다음 그랄까지 얼마나 찼는지 — 체육관 기준이 맞을 때만 켜세요 |
| ✏️ **수련 메모** | 배운 기술·세미나·대회·본 영상을 그날 날짜에 |
| 📸 **공유 카드** | 내 띠를 이미지 한 장으로 |
| ☁️ **자동 백업** | 깃허브 계정만 있으면 폰·PC가 같은 기록 |
| ✈️ **오프라인** | 지하 체육관에서도 그냥 열립니다 |
...
매일 열 거니까 앱처럼 두는 게 편해요. 주소창 없이 뜨고, **한 번 열어두면 그다음부터는
인터넷이 없어도 열립니다** — 지하 체육관에서도 그냥 찍으면 돼요.

- 아이폰: 사파리로 열고 → 공유 → **홈 화면에 추가**
- 안드로이드: 크롬으로 열고 → 메뉴 → **앱 설치** (또는 홈 화면에 추가)
...
- **잘못 지웠어도 괜찮아요.** 취소하면 아래에 `되돌리기` 가 잠깐 뜹니다. 놓쳤어도
  그 날짜를 다시 누르면 그만이고요
...
- **잔디** — 최근 1년. 빈칸 많아도 괜찮아요, 채워가는 재미니까.
  ...
  **칸을 누르면 그 달로 달력이 넘어갑니다** — 「이 근처에 한참 쉬었네」 싶을 때 바로 가서 보세요
...
**어제 일을 적고 싶으면 달력에서 그 날짜를 꾹 누르세요** — 바로 그날 메모가 열립니다.
(짧게 누르면 출석, 길게 누르면 메모. PC에서는 오른쪽 클릭이에요)
...
기록해두면 맨 위 벨트가 바뀌고, 달력에 주황 점이, 잔디에 노란 칸이 남습니다.
**이건 아래 「승급 진행도 추적」과 상관없이 언제나 동작해요.**
...
이건 **기본으로 꺼져 있어요.** 승급 기준은 체육관마다 다른데, 이 앱에 들어 있는 건
한 곳의 기준이라 다른 데 다니시면 숫자가 그냥 틀리거든요. 남의 체육관 기준으로
「D-72」라고 알려주는 것만큼 쓸데없는 것도 없으니까요.

`설정` → **승급 진행도 추적** 을 켜면 나옵니다.

<img src="docs/images/tracking.png" width="360" alt="승급 진행도 추적 켜고 끄는 설정 스위치">

> **승급 이력이랑은 별개예요.** 추적을 꺼 둬도 그랄 받은 날은 얼마든지 기록할 수 있고,
> 벨트 표시·달력의 승급일 점·잔디의 승급 칸은 그대로 나옵니다.
> 껐다고 뭐가 지워지지도 않아요 — 나중에 다시 켜면 그동안 쌓인 걸로 바로 계산됩니다.
...
|---|---|
| 맨 위 벨트 | 지금 내 벨트랑 그랄 |
| 달력 | 짧게 = 출석 체크, 길게 = 그날 메모. 초록이 나간 날, 칸 아래 짧은 줄은 메모 있는 날 |
| 달력의 주황 점 | 그날 승급했다는 표시 |
| 달력의 어두운 초록 | 마지막 승급 전에 나간 날 |
| 달력 아래 목록 | 그 달에 적어둔 메모. 달을 넘기면 같이 넘어감 |
| 기록 카드 | 총 몇 번, 몇 주 연속, 요즘 주당 몇 회 |
| 잔디 | 최근 1년 출석. 어두운 칸은 앱 쓰기 전, 노란 칸은 승급일. 누르면 그 달로 이동 |

아래 셋은 **승급 진행도 추적을 켰을 때만** 나옵니다.
...
(승급 이력과 분류도 같아요). 같은 날 메모를 양쪽에서 고쳤다면 **나중에 저장한 쪽**이 남아요.

**Q. 인터넷이 안 되는 곳에서도 되나요?**
한 번 열어본 기기라면 **그냥 열립니다.** 출석도 메모도 평소처럼 찍히고, 깃허브에 연결해
두셨다면 신호가 잡히는 순간 알아서 올라가요.
...
**Q. 도장 승급 기준이 달라요.**
기준 자체는 아직 위 표(3개월/30일 · 7개월/90일)로 고정이에요. 대신 **승급 진행도 추적을
꺼 두시면** 됩니다 — 기본값이 꺼짐이라 아무것도 안 하셔도 되고요.
출석·연속·잔디·메모는 어느 체육관이든 그대로 쓸모 있고, 승급 이력도 그대로 남습니다.

**Q. 추적을 껐다 켜면 그동안 기록이 날아가나요?**
아니요. 끄는 건 **화면에서 감추는 것뿐**이라 출석도 승급 이력도 그대로 있습니다.
다시 켜면 그동안 쌓인 걸로 바로 계산돼요.

---

오스! 🥋

<sub>앱을 고치거나 데이터를 직접 다루려면 → [`docs/implementation.md`](docs/implementation.md) ·
[`docs/data-format.md`](docs/data-format.md) · [`docs/decisions/`](docs/decisions/) ·
[`docs/lessons-learned.md`](docs/lessons-learned.md)</sub>
