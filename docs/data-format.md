...
#### 사전으로 쓰는 객체에는 프로토타입이 없어야 한다

`checked` · `removed` · `removedHistory` · `removedNotes` · `removedTags` 는 **키가 데이터인
사전**이다. 특히 `removedTags` 의 키는 사용자가 정하는 분류 id 라 `toString` · `valueOf` ·
`__proto__` 가 될 수 있다 (전부 10자 이하다). 평범한 `{}` 로 만들면 세 가지가 한꺼번에 깨진다.

| 증상 | 원인 |
|---|---|
| 지운 적 없는 분류가 사라진다 | `map["toString"]` 이 `Object.prototype` 의 함수를 돌려줘 툼스톤이 있는 것처럼 보인다 |
| 삭제가 전파되지 않는다 | `map["__proto__"] = "..."` 은 setter 라 조용히 무시된다 |
| 멀쩡한 백업이 거부된다 | `"toString" in map` 이 항상 참이라 §9.1 의 중복 검사가 걸린다 |

그래서 이 앱은 사전을 `Object.create(null)` 로 만들고, 검사에는 `in` 대신
`Object.hasOwn()` 을 쓴다. **다른 도구로 이 데이터를 읽을 때도 같은 주의가 필요하다** —
JSON 으로 저장되는 모양은 평범한 객체와 완전히 같으므로 읽는 쪽에서 다시 밟기 쉽다.
(Python 의 `dict` 에는 해당 없음)

#### 길이 상한은 글자를 쪼개지 않는다

`text` 500자 · 분류 이름 10자는 **UTF-16 코드 유닛** 기준이다 (화면의 `N / 500` 과
`maxlength` 가 그 단위여야 맞는다). 이모지는 두 유닛이라 경계가 그 사이에 떨어질 수 있는데,
그대로 자르면 반쪽 글자(`�`)가 남는다. 자를 때 마지막이 서러게이트 상반이면 한 칸 앞에서 끊는다.
